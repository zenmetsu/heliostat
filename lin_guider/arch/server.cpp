...
void server::return_conn( conn_t *cn )
{
	...
	if( wr_res == -1 )
	{
		printf("server::return_conn(): Unable to write to pipe: '%s'", strerror(errno) );
	}
}
...
bool server::send_broadcast( const char * str, size_t str_len )
{
	...
	{
		printf( "server::send_broadcast(): socket error" );
		return false;
	}
	...
	{
		printf( "server::send_broadcast(): setsockopt error");
		close( sock );
		return false;
	}
	...
	{
		printf( "server::send_broadcast(): sendto error" );
		close( sock );
		return false;
	}
...
int server::add_epoll_event( int fd, void *data, int eventfl )
{
	...
	if( epoll_ctl(m_epollfd, EPOLL_CTL_ADD, fd, &event) )
	{
		...
		printf("ADD_EPOLL_EVENT ERROR (%d): %s", errno, err_buf);
		return -1;
	}
	return 0;
}
...
int server::mod_epoll_event( int fd, void *data, int eventfl )
{
	...
	{
		printf("MOD_EPOLL_EVENT ERROR (%d,%d,%d,%d): %s", errno, m_epollfd, fd, eventfl, strerror(errno));
		return -1;
	}
	return 0;
}
...
int server::del_epoll_event( int fd )
{
	...
	{
		printf("EP: delete connection from epoll failed: %s, %d, %d", strerror( errno), errno, fd);
		return -1;
	}
	return 0;
}
...
int  server::close_conn( conn_t *conn )
{
	...
	printf( "Deleted[%d], opened: %d conns", old_fd, m_conns.size() );
...
int  server::do_read( conn_t *pconn )
{
	...
	else
	{
		...
		if( pconn->read_pos >= proto_hdr::HDR_SIZE && pconn->read_len == 0 )
		{
			...
			{
				printf( "SRV: Protocol error. Signature is wrong" );
				ret = close_conn( pconn );
				return ret;
			}
			std::map< int, const char* >::const_iterator cmd = m_cmd_names.find( r_hdr->cmd );
			printf( "SRV: header parsed: cmd = '%s' read_len = %d", cmd != m_cmd_names.end() ? cmd->second : "UNKNOWN", pconn->read_len );
			if( pconn->read_len > (ssize_t)sizeof(pconn->read_buf) )
			{
				pconn->read_len = sizeof(pconn->read_buf);
				printf( "SRV: Data stripped to = %d", sizeof(pconn->read_buf) );
			}
		}
...
void *server::server_thread( void *param )
{
	...
	if (m_net_params.use_tcp) {
		printf("SRV: listening on TCP port %d", m_net_params.listen_port);
	} else {
		printf("SRV: listening on Unix socket %s", m_net_params.listen_socket);
	}
    ...
    if( srv->m_res_fd < 0 )
    {
        printf( "server::server_thread(): eventfd() failed. '%s'", strerror( errno ) );
    }
    else
    {
	...
	if( ret == -1)
  		...
  		for( int n = 0;n < nfds;n++ )
  		{
  			...
  			if( ev->data.fd == listen_sock )
  			{
				...
				conn_sock = accept(listen_sock,	cli_addr, &clen);
  				...
  				{
  					printf( "Event array is full. conn.cnt = %d", srv->m_conns.size() );
  					close( conn_sock );
  					continue;
  				}
  				...
  				if( ret == -1 )
  					exit(EXIT_FAILURE);
  				printf( "Connected[%d], opened: %d conns", pconn->fd, srv->m_conns.size() );
  			}
  			else
 	...
 	printf( "SRV:in_queue.size(): %zu", srv->m_in_queue.size() );
 	 	...
 	 	++it )
	{
 	...
 	pthread_mutex_lock( &srv->m_out_mtx );
 	printf( "SRV:out_queue.size(): %zu", srv->m_out_queue.size() );
